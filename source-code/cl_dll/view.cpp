...
void V_CalcBob(struct ref_params_s* pparams, float freqmod, calcBobMode_t mode, double& bobtime, float& bob, float& lasttime)
{
	...
	// flbob is proportional to simulated velocity in the xy plane
...
//	pparams->vieworg[2] += bobUp;
	...
	VectorMA(view->origin, g_verticalBob * 0.15f, forward, view->origin);

	// Z bob a bit more		
	view->origin[2] += g_verticalBob * 0.15f;

	// bob the angles	
	angles[ROLL] += g_verticalBob * 0.4f;
	angles[PITCH] -= g_verticalBob * 0.85f;
	angles[YAW] -= g_lateralBob * 0.85f;

	VectorMA(view->origin, g_lateralBob * 0.8f, right, view->origin);

	if (cl_headbob_enable->value > 0)
	{
		//funny head bob
		pparams->viewangles[ROLL] += g_verticalBob * 0.05;
		pparams->viewangles[PITCH] += g_lateralBob * 0.19;
	}
...
void V_Init ()
{
	...
	cl_headbob_enable = gEngfuncs.pfnRegisterVariable("cl_headbob_enable", "1", FCVAR_ARCHIVE);
}
