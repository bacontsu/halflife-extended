...
void CGrenade::Explode( TraceResult *pTrace, int bitsDamageType )
{
	...
	MESSAGE_BEGIN( MSG_PAS, SVC_TEMPENTITY, pev->origin );
		...
		WRITE_BYTE( (pev->dmg - (pev->dmg * 0.5)) * .60  ); // scale * 10
...
void CGrenade::Smoke()
{
	...
	else
	{
		MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, pev->origin );
			...
			WRITE_BYTE( (pev->dmg - (pev->dmg * 0.4)) * 0.80 ); // scale * 10
...
void CGrenade::BounceExplodeTouch( CBaseEntity *pOther )
{
	...
	{
		// play bounce sound
		EMIT_SOUND(ENT(pev), CHAN_VOICE, "weapons/bounce.wav", 0.25, ATTN_NORM);
	}
...
CGrenade* CGrenade::ShootBouncy(entvars_t* pevOwner, Vector vecStart, Vector vecVelocity, float time)
{
	...
	pGrenade->pev->avelocity = Vector(200, 200, 200);
