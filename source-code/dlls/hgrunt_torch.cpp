...
#define TORCH_DEAGLE 0
#define TORCH_GLOCK 1
#define TORCH_BLOWTORCH 2
#define TORCH_NOGUN 3
...
//  DEFINE_FIELD( CShotgun, m_iShotgunShell, FIELD_INTEGER ),
	...
	DEFINE_FIELD(COFTorchAlly, m_iWeaponIdx, FIELD_INTEGER),
	DEFINE_FIELD(COFTorchAlly, m_iGruntHead, FIELD_INTEGER),
};
...
void COFTorchAlly :: GibMonster ()
{
	...
	if (GetBodygroup(TorchAllyBodygroup::Weapons) == TorchAllyWeapon::Glock)
	{// throw a gun if the grunt has one
		GetAttachment(0, vecGunPos, vecGunAngles);
...
{
	if ( m_fGunHolstered || m_hasHandGrenades != TRUE )
	{
		return FALSE;
	}
...
// the enemy the torch is facing.
//=========================================================
void COFTorchAlly::FirePistol()
{
...
void COFTorchAlly :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		case TORCH_AE_HOLSTER_TORCH:
			{
			if (pev->weapons == 0)//deagle
				SetBodygroup(TorchAllyBodygroup::Weapons, TorchAllyWeapon::DesertEagle);
			if (pev->weapons == 1)//glock
				SetBodygroup(TorchAllyBodygroup::Weapons, TorchAllyWeapon::Glock);
		...
		{
			if(pev->weapons == 0)//deagle
				SetBodygroup(TorchAllyBodygroup::Weapons, TorchAllyWeapon::Torch_deagle);
			if (pev->weapons == 1)//glock
				SetBodygroup(TorchAllyBodygroup::Weapons, TorchAllyWeapon::Torch_glock);
		...
		case TORCH_AE_HOLSTER_BOTH:
			{
			if (pev->weapons == 0)//deagle
				SetBodygroup(TorchAllyBodygroup::Weapons, TorchAllyWeapon::DesertEagle_holstered);
			if (pev->weapons == 1)//glock
				SetBodygroup(TorchAllyBodygroup::Weapons, TorchAllyWeapon::Glock_holstered);
				m_fGunHolstered = true;
				m_fTorchHolstered = true;
				break;
			}
...
void COFTorchAlly :: Spawn()
{
	...
	SET_MODEL(ENT(pev), "models/hgrunt_medtorch.mdl");
	...
	if (pev->weapons == 1)
	{
		weaponIndex = TorchAllyWeapon::Glock;
		m_cClipSize = 12;
	}
	if (pev->weapons == 3)
	{
		weaponIndex = TorchAllyWeapon::None;
		m_cClipSize = -1;
	}
...
void COFTorchAlly::KeyValue(KeyValueData* pkvd)
{
