...
void CHoundeye :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch ( pEvent->event )
	{
		...
		case HOUND_AE_STARTATTACK:
			if (FClassnameIs(pev, "monster_houndeye_baby"))
				pev->framerate = 1.5;
			WarmUpSound();
			break;
...
void CHoundeye :: Precache()
{
	...
	PRECACHE_MODEL("models/gibs/houndeye_gibs.mdl");
...
// Baby Houndeye
//================================================================
class CBabyHound : public CHoundeye
{
public:
	void Spawn();
	void Precache() override;
...
LINK_ENTITY_TO_CLASS(monster_houndeye_baby, CBabyHound);

//=======================================
// Spawn
//=======================================
void CBabyHound::Spawn()
{
	Precache();

	SET_MODEL(ENT(pev), "models/baby_houndeye.mdl");
	...
	m_bloodColor = BLOOD_COLOR_YELLOW;
	pev->effects = 0;
	pev->health = gSkillData.houndeyeHealth / 2;
	pev->yaw_speed = 5;//!!! should we put this in the monster's changeanim function since turn rates may vary with state/anim?
	m_flFieldOfView = 0.5;// indicates the width of this monster's forward view cone ( as a dotproduct result )
	m_MonsterState = MONSTERSTATE_NONE;
	m_fAsleep = FALSE; // everyone spawns awake
	...
	m_voicePitch = RANDOM_LONG(115, 135);
...
// Precache - precaches all resources this monster needs
//=========================================================
void CBabyHound::Precache()
{
	PRECACHE_MODEL("models/baby_houndeye.mdl");
...
void CBabyHound::Killed(entvars_t* pevAttacker, int iGib)
{
