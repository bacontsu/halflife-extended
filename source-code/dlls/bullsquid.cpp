...
int CBullsquid::IRelationship ( CBaseEntity *pTarget )
{
	...
	if (FClassnameIs(pTarget->pev, "monster_houndeye_baby"))
	{
		return R_HT;
	}
...
{
	int iPitch;
	if (pev->body == SQUID_MALE)
	{
		iPitch = RANDOM_LONG(85, 100);
	}
	if (pev->body == SQUID_FEMALE)
	{
		iPitch = RANDOM_LONG(95, 130);
	}
...
void CBullsquid :: AlertSound ()
{
	...
	if (pev->body == SQUID_MALE)
	{
		iPitch = RANDOM_LONG(90, 120);
	}
	if (pev->body == SQUID_FEMALE)
	{
		iPitch = RANDOM_LONG(120, 170);
	}
...
void CBullsquid :: Precache()
{
	...
	PRECACHE_MODEL("models/gibs/bullsquid_gibs.mdl");
...
// Dead Squid PROP
//=========================================================
class CDeadSquid : public CBaseMonster
{
public:
	void Spawn() override;
...
const char* CDeadSquid::m_szPoses[] = { "dead_front", "dead_flip" };

void CDeadSquid::KeyValue(KeyValueData* pkvd)
{
...
LINK_ENTITY_TO_CLASS(monster_bullchicken_dead, CDeadSquid);

//
// ********** DeadSquid SPAWN **********
//
void CDeadSquid::Spawn()
{
	PRECACHE_MODEL("models/bullsquid.mdl");
	SET_MODEL(ENT(pev), "models/bullsquid.mdl");
	...
	{// -1 chooses a random head
		pev->body = RANDOM_LONG(0, 1);// pick a body, any body
	}

	pev->sequence = LookupSequence(m_szPoses[m_iPose]);
	if (pev->sequence == -1)
	{
		ALERT(at_console, "Dead Squid with bad pose\n");
	}
