...
void CZombieBarney :: Spawn()
{
	...
	{// -1 chooses a random body
		pev->body = RANDOM_LONG(0, 2);// pick a body, any body
	}
	if (pev->skin == -1)
	{// -1 chooses a random skin
...
void CZombieBarney::Killed(entvars_t* pevAttacker, int iGib)
{
	if (pev->spawnflags & SF_DROP_AMMO)
	{
		if(pev->weapons == 0)//drop glock ammo
			DropItem("ammo_9mmclip", Vector(pev->origin - Vector(0, 0, 20)), pev->angles);
		if (pev->weapons == 1)//drop python/deagle ammo
			DropItem("ammo_357", Vector(pev->origin - Vector(0, 0, 20)), pev->angles);
	}
