...
TYPEDESCRIPTION	CBasePlayer::m_playerSaveData[] =
{
	...
	DEFINE_FIELD(CBasePlayer, m_pHoldable, FIELD_EHANDLE),
...
void CBasePlayer::RemoveAllItems(BOOL removeSuit)
{
	...
	if (m_pHoldable != NULL)
	{
		m_pHoldable->Use(this, this, USE_OFF, 0);
		m_pHoldable = NULL;
	}
...
void CBasePlayer::PostThink()
{
	...
	if (m_pHoldable != NULL)
	{
		...
		Vector vecDest = pev->origin + pev->view_ofs + gpGlobals->v_forward * PLAYER_SEARCH_RADIUS;

		Vector vecDiff = vecDest - VecBModelOrigin(m_pHoldable->pev);

		float fDistFactor = vecDiff.Length() * 24.0f;

		if (vecDiff != g_vecZero)
			m_pHoldable->pev->velocity = vecDiff.Normalize() * fDistFactor;

		if (vecDiff.Length() > 72.0f)
		{
			m_pHoldable->Use(this, this, USE_OFF, 0);
			m_pHoldable = NULL;
		}
...
void CBasePlayer::CheatImpulseCommands(int iImpulse)
{
	...
	case 101:
		...
		GiveNamedItem("weapon_chummy");
		//	GiveNamedItem( "weapon_beretta" );
		//	GiveNamedItem( "weapon_baton" );
