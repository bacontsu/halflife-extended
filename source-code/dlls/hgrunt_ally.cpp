...
BOOL CHGruntAlly :: CheckRangeAttack1 ( float flDot, float flDist )
{
	...
	if( pev->weapons )
	{
		const auto maxDistance = pev->weapons == 2 ? 640 : 1024;
...
void CHGruntAlly :: Spawn()
{
	...
	if (m_hasHandGrenades != 1 && m_hasHandGrenades != 2)//if 'Has grenades' is neither 1(yes) nor 2(no)...
	{
		...
		ALERT(at_console, "monster_hgrunt_ally with exotic 'Has grenades' parameter, choosing randomly... \n");
