...
class CShotgunTurret : public CTurret
{
	void Precache() override;
	void Shoot(Vector& vecSrc, Vector& vecDirToEnemy) override;
};

LINK_ENTITY_TO_CLASS(monster_turret_buckshot, CShotgunTurret);
...
void CShotgunTurret::Precache()
{
	CBaseTurret::Precache();
	PRECACHE_MODEL("models/turret.mdl");
	PRECACHE_MODEL(TURRET_GLOW_SPRITE);
	PRECACHE_SOUND("weapons/sbarrel_alt.wav");
}
...
void CShotgunTurret::Shoot(Vector& vecSrc, Vector& vecDirToEnemy)
{
	FireBullets(6, vecSrc, vecDirToEnemy, VECTOR_CONE_10DEGREES, TURRET_RANGE, BULLET_PLAYER_BUCKSHOT, 1);
	EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/sbarrel_alt.wav", 1, 0.6);
	pev->effects = pev->effects | EF_MUZZLEFLASH;
}
...
void CBaseTurret::SearchThink()
{
	...
	if (m_hEnemy != NULL)
	{
		if (m_hEnemy->pev->deadflag == DEAD_DEAD)
