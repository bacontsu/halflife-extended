...
// barnacle has prey.
	...
	// still pulling prey.
		...
		// prey has just been lifted into position ( if the victim origin + eye height + 8 is higher than the bottom of the barnacle, it is assumed that the head is within barnacle's body )
				...
				if (!pVictim->IsPlayer())// non-clients die much faster
					m_flKillVictimTime = gpGlobals->time + 3;
				else m_flKillVictimTime = gpGlobals->time + 10;// now that the victim is in place, the killing bite will be administered in 10 seconds.
	...
	// prey is lifted fully into feeding position and is dangling there.
			...
			if ( m_flKillVictimTime != -1 && gpGlobals->time > m_flKillVictimTime )
			{
				...
				if ( pVictim )
				{
					pVictim->TakeDamage ( pev, pev, pVictim->pev->health + 10, DMG_SLASH | DMG_ALWAYSGIB );
					m_cGibs = RANDOM_LONG(0,2) + 3;
				}
