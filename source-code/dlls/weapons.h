...
#define SetWeaponSkin(x) if (m_pPlayer) { SWP_BASE(x); } pev->skin = x; iWeaponSkin = x;
#endif // CLIENT
...
#define	WEAPON_CHUMMY			27
//#define	WEAPON_BATON			xx <- BS:E exclusive
//#define	WEAPON_BERETTA			xx <- BS:E exclusive
#define	WEAPON_AR			28
#define	WEAPON_M79			29
...
typedef	enum
{
	...
	BULLET_PLAYER_556, // SAW and Assault rifle
	BULLET_PLAYER_762,
	BULLET_PLAYER_EAGLE,
} Bullet;
...
public:
	...
	int		iWeaponSkin;										//our weapon's current skin, updated via SetWeaponSkin macro	
...
public:
	...
	void PrimaryAttack() override;
	void SecondaryAttack() override;
	void ARFire(float flSpread, float flCycleTime, BOOL fUseAutoAim, int iVolume);
	BOOL Deploy() override;
...
public:
	...
	static CFireball* CreateFireball(Vector vecOrigin, Vector vecAngles, CBaseEntity* pOwner, CRpg* pLauncher);

	int m_iTrail;
	float m_flIgniteTime;
	EHANDLE m_pFireLauncher;// handle back to the launcher that fired me. 
};
...
public:
	...
	//texture animation ~ bacontsu
	int mBody = 0;
	float nextBlinkUpdate = 0.0f;
...
class CChub : public CBasePlayerWeapon
{
public:
	void Spawn() override;
	void Precache() override;
