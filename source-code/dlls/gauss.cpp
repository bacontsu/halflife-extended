...
class CGaussAmmoBig : public CBasePlayerAmmo
{
	void Spawn() override
	{
		Precache();
	...
	void Precache() override
	{
		...
		PRECACHE_SOUND("items/scifi_pickup.wav");
	}
	BOOL AddAmmo(CBaseEntity* pOther) override
	{
		if (pOther->GiveAmmo(100, "uranium", URANIUM_MAX_CARRY) != -1)
		{
			EMIT_SOUND(ENT(pev), CHAN_ITEM, "items/scifi_pickup.wav", 2, ATTN_NORM);
			return TRUE;
		}
		return FALSE;
	}
};
LINK_ENTITY_TO_CLASS(ammo_isotope_box, CGaussAmmoBig);
