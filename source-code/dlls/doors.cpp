...
public:
	...
	//void PlayCustomLockSounds(entvars_t* pev, locksound_t* pls, int flocked, int fbutton);
	...
	string_t CustLockedSnd;
	string_t CustUnLockedSnd;

	float nextCustomSndTime;
};
...
TYPEDESCRIPTION	CBaseDoor::m_SaveData[] = 
{
	...
	//DEFINE_FIELD(CBaseDoor, CustLockedSnd, FIELD_CHARACTER),
	//DEFINE_FIELD(CBaseDoor, CustUnLockedSnd, FIELD_CHARACTER),
	DEFINE_FIELD(CBaseDoor, nextCustomSndTime, FIELD_FLOAT),
	//DEFINE_FIELD(CBaseDoor, m_bStopSnd, FIELD_CHARACTER),
...
void CBaseDoor::KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "CustLockedSnd"))
	{
		CustLockedSnd = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "CustUnLockedSnd"))
	{
		CustUnLockedSnd = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else
...
// set the door's "in-motion" sound
	...
	case	11:
		...
		pev->noiseMoving = ALLOC_STRING("debris/bustconcrete1.wav");
		break;
	case	12:
		PRECACHE_SOUND("debris/bustconcrete2.wav");
		pev->noiseMoving = ALLOC_STRING("debris/bustconcrete2.wav");
		break;
	case	13:
		PRECACHE_SOUND("debris/concrete1.wav");
		pev->noiseMoving = ALLOC_STRING("debris/concrete1.wav");
		break;
	case	14:
		PRECACHE_SOUND("debris/concrete2.wav");
		pev->noiseMoving = ALLOC_STRING("debris/concrete2.wav");
		break;
	case	15:
		PRECACHE_SOUND("debris/concrete3.wav");
		pev->noiseMoving = ALLOC_STRING("debris/concrete3.wav");
		break;
	default:
		pev->noiseMoving = ALLOC_STRING("common/null.wav");
		break;
...
// set the door's 'reached destination' stop sound
	...
	case	10:
		PRECACHE_SOUND("debris/bustconcrete2.wav");
		pev->noiseArrived = ALLOC_STRING("debris/bustconcrete2.wav");
		break;
	case	11:
		PRECACHE_SOUND("debris/concrete1.wav");
		pev->noiseArrived = ALLOC_STRING("debris/concrete1.wav");
		break;
	case	12:
		PRECACHE_SOUND("debris/concrete2.wav");
		pev->noiseArrived = ALLOC_STRING("debris/concrete2.wav");
		break;
	case	13:
		PRECACHE_SOUND("debris/concrete3.wav");
		pev->noiseArrived = ALLOC_STRING("debris/concrete3.wav");
		break;
	default:
		pev->noiseArrived = ALLOC_STRING("common/null.wav");
		break;
	...
	if (!FStringNull(CustLockedSnd))
	{
		PRECACHE_SOUND(STRING(CustLockedSnd));
	}
	if (!FStringNull(CustUnLockedSnd))
	{
		PRECACHE_SOUND(STRING(CustUnLockedSnd));
	}
}
...
void CBaseDoor::DoorTouch( CBaseEntity *pOther )
{
	...
	if (m_sMaster && !UTIL_IsMasterTriggered(m_sMaster, pOther))
	{
		...
		if (!FStringNull(CustLockedSnd))
		{
			if (nextCustomSndTime <= gpGlobals->time)
			{
				EMIT_SOUND_DYN(ENT(pev), CHAN_ITEM, STRING(CustLockedSnd), VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
				nextCustomSndTime = gpGlobals->time + 3;
			}
		}
		else PlayLockSounds(pev, &m_ls, TRUE, FALSE);
	}
...
int CBaseDoor::DoorActivate( )
{
	...
	{// door should open
		...
		if (!FStringNull(CustUnLockedSnd))
		{
			if (nextCustomSndTime <= gpGlobals->time)
			{
				EMIT_SOUND_DYN(ENT(pev), CHAN_ITEM, STRING(CustUnLockedSnd), VOL_NORM, ATTN_NORM, 0, PITCH_NORM);
				nextCustomSndTime = gpGlobals->time + 3;
			}
		}
		else PlayLockSounds(pev, &m_ls, FALSE, FALSE);
