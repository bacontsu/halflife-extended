...
#define HGRUNT_NUM_HEADS				20 // how many grunt heads are there? 
#define HGRUNT_MINIMUM_HEADSHOT_DAMAGE	15 // must do at least this much damage in one shot to head to score a headshot kill
#define	HGRUNT_SENTENCE_VOLUME			(float)0.35 // volume of grunt sentences

#define HGRUNT_MP5 0;//MP5
...
#define		HGRUNT_AE_DROP_GUN		( 11) // grunt (probably dead) is dropping his gun.
...
TYPEDESCRIPTION	CHGrunt::m_SaveData[] = 
{
	...
	DEFINE_FIELD(CHGrunt, m_flNextRocketCheck, FIELD_TIME),
	DEFINE_FIELD(CHGrunt, m_fFireRocket, FIELD_TIME),
...
//const GibData HGruntGibs = { "models/gibs/hgruntgibs.mdl", 0, 8 };

//=========================================================
// GibMonster - make gun fly through the air.
//=========================================================
const GibData HECUGibGasmask = { "models/gibs/gib_hgrunt.mdl", 2, 3 };
const GibData HECUGibBeret = { "models/gibs/gib_hgrunt.mdl", 3, 3 };
...
BOOL CHGrunt :: CheckMeleeAttack1 ( float flDot, float flDist )
{
	...
	if ( flDist <= 64 && flDot >= 0.7	&& 
		 ...
		 pEnemy->Classify() != CLASS_CHUMTOAD  )
	{
		return TRUE;
	}
	return FALSE;
...
BOOL CHGrunt :: CheckRangeAttack1 ( float flDot, float flDist )
{
	...
	if (pev->weapons == 5)
	{
		...
		// are any of my squad members near the intended rocket impact area?
		if (InSquad())
		{
			if (SquadMemberInRange(vecTarget, 512))
			{
				// crap, I might blow my own guy up. Don't fire a rocket and don't check again for a while.
				m_flNextRocketCheck = gpGlobals->time + 1; // one full second.
				m_fFireRocket = FALSE;
			}
		}

		if ((vecTarget - pev->origin).Length2D() <= 512)
		{
			// crap, I don't want to blow myself up
			m_flNextRocketCheck = gpGlobals->time + 1; // one full second.
			m_fFireRocket = FALSE;
			return m_fFireRocket;
		}
	}
...
{
	if (pev->weapons != 1 && m_hasHandGrenades != TRUE)
	{
		return FALSE;
	}
...
void CHGrunt :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	...
	if (ptr->iHitgroup == 11)
	{
		...
		if (m_hasHelmet == TRUE && (bitsDamageType & (DMG_BULLET | DMG_SLASH | DMG_BLAST | DMG_CLUB)))
		{
			// absorb damage
			flDamage -= 20;
			if (flDamage <= 0)
...
void CHGrunt::ShootSaw()
{
	...
	Vector	vecShellVelocity = gpGlobals->v_right * RANDOM_FLOAT(40, 90) + gpGlobals->v_up * RANDOM_FLOAT(75, 200) + gpGlobals->v_forward * RANDOM_FLOAT(-40, 40);
	EjectBrass(vecShootOrigin - vecShootDir * 24, vecShellVelocity, pev->angles.y, m_iShotgunShell, TE_BOUNCE_SHOTSHELL);
	FireBullets(1, vecShootOrigin, vecShootDir, VECTOR_CONE_15DEGREES, 2048, BULLET_PLAYER_9MM, 4, 6); // shoot +-7.5 degrees
...
void CHGrunt::ShootRpg()
{
	...
	Vector vecShootOrigin = GetGunPosition();
	Vector vecShootDir = ShootAtEnemy(vecShootOrigin);
	Vector vecEnemy = (pev->angles + pEnemy->pev->origin) - m_vecEnemyLKP;//using vecShootDir makes me always fire in the Eastern direction for some reason, so I will calculate my aim like this

	UTIL_MakeVectors(pev->angles);

	CBaseEntity* pRocket = CBaseEntity::Create("rpg_rocket", vecShootOrigin, vecEnemy, edict());
...
void CHGrunt :: Spawn()
{
	...
	if (m_hasHandGrenades != 1 && m_hasHandGrenades != 2)//if 'Has grenades' is neither 1(yes) nor 2(no)...
	{
		m_hasHandGrenades = RANDOM_LONG(1, 2);
		ALERT(at_console, "monster_hgrunt with exotic 'Has grenades' parameter, choosing randomly... \n");
		if (m_hasHandGrenades == 1)
		{
			ALERT(at_console, "... Will have grenades \n");
		}
		else ALERT(at_console, "... Won't have grenades \n");
		
		if (m_iGruntHead == 0 && m_SpawnChance <= 0)// all these parameters are empty, probably a deprecated hgrunt, randomise...
		{
			m_iGruntHead = Head::Random;
			pev->skin = -1;
		}
	}
...
{
	if (FStrEq(pkvd->szKeyName, "grenades"))
	{
		m_hasHandGrenades = atoi(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
...
void CHGrunt :: SetActivity ( Activity NewActivity )
{
	...
	case ACT_RANGE_ATTACK1:
		...
		if (pev->weapons == 5)
		{
			...
			// get crouching shoot
			iSequence = LookupSequence("rpg_shoot");
...
// can grenade launch
			...
			else if ( pev->weapons == 1 && HasConditions ( bits_COND_CAN_RANGE_ATTACK2 ) && OccupySlot( bits_SLOTS_HGRUNT_GRENADE ) )
			{
...
void CHGruntRepel::RepelUse ( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	*/
	...
	auto pGrunt = static_cast<CHGrunt*>(pEntity->MySquadMonsterPointer());
