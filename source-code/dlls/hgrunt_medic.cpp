...
//  DEFINE_FIELD( CShotgun, m_iShotgunShell, FIELD_INTEGER ),
	...
	DEFINE_FIELD( COFMedicAlly, m_hasHandGrenades, FIELD_BOOLEAN ),
...
void COFMedicAlly :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	switch( pEvent->event )
	{
		...
		case MEDIC_AE_HOLSTER_GUN:
			if (pev->weapons == 1)//deagle
				SetBodygroup(MedicAllyBodygroup::Weapons, MedicAllyWeapon::DesertEagle_holstered);
			if (pev->weapons == 2)//glock
				SetBodygroup(MedicAllyBodygroup::Weapons, MedicAllyWeapon::Glock_holstered);
		...
		case MEDIC_AE_EQUIP_NEEDLE:
			if (pev->weapons == 1)//deagle
				SetBodygroup(MedicAllyBodygroup::Weapons, MedicAllyWeapon::Needle_deagle);
			if (pev->weapons == 2)//glock
				SetBodygroup(MedicAllyBodygroup::Weapons, MedicAllyWeapon::Needle_glock);
		...
		case MEDIC_AE_EQUIP_GUN:
			SetBodygroup( MedicAllyBodygroup::Weapons, pev->weapons == 2 ? MedicAllyWeapon::Glock : MedicAllyWeapon::DesertEagle );
...
void COFMedicAlly :: Spawn()
{
	...
	if( !pev->weapons || pev->weapons == -1)
	{
		pev->weapons = RANDOM_LONG(1,2);
	}
	if (pev->body == -1)
	{
		pev->body = RANDOM_LONG(0, 3) + 4;
	}
	if (pev->skin == -1)
	{
