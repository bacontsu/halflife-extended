...
void CEagle::WeaponIdle()
{
	...
	if (m_bLaserActive)
	{
		if (nextAnimUpdate <= gpGlobals->time + 0.1f && nextAnimUpdate > gpGlobals->time)
		{
			SetWeaponSkin(1);
		}
		else if (nextAnimUpdate <= gpGlobals->time)
		{
			SetWeaponSkin(0);
		}
	}
...
void CEagle::PrimaryAttack()
{
	...
	if( m_pLaser && m_bLaserActive )
	{
		...
		m_pLaser->pev->nextthink = UTIL_WeaponTimeBase() + 0.5;
	}
#endif
...
void CEagle::SecondaryAttack()
{
	...
	nextAnimUpdate = gpGlobals->time + 0.2f;
...
void CEagle::Reload()
{
	...
	{
		//turn off laser during reload anim
		if (nextAnimUpdate <= gpGlobals->time + 0.1f && nextAnimUpdate > gpGlobals->time)
		{
			SetWeaponSkin(1);
		}
	...
	int ReloadAnim;
	int bResult;
	if (!m_iClip) ReloadAnim = EAGLE_RELOAD_NOSHOT;
	else ReloadAnim = EAGLE_RELOAD;
	bResult = DefaultReload( EAGLE_MAX_CLIP, ReloadAnim, 1.5, 1 );
