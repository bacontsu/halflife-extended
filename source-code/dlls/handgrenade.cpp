...
#define	HANDGRENADE_ANIM_SHORTTHROW		8
...
void CHandGrenade::SecondaryAttack()
{
	if (!m_flStartThrow && m_pPlayer->m_rgAmmo[m_iPrimaryAmmoType] > 0)
	{
		...
		SendWeaponAnim(HANDGRENADE_ANIM_SHORTTHROW);
...
void CHandGrenade::WeaponIdle()
{
	...
	if ( m_flStartThrow )
	{
		...
		if ( !m_pPlayer->m_rgAmmo[ m_iPrimaryAmmoType ] )
		{
			...
			m_flTimeWeaponIdle = m_flNextPrimaryAttack = GetNextAttackDelay(0.5);// ensure that the animation can finish playing
		}
		return;
	}
...
class CGrenadeBox : public CBasePlayerAmmo
{
	void Spawn() override
	{
		Precache();
		SET_MODEL(ENT(pev), "models/w_ammo_handgrenade_box.mdl");
		CBasePlayerAmmo::Spawn();
	}
	void Precache() override
	{
		PRECACHE_MODEL("models/w_ammo_handgrenade_box.mdl");
...
LINK_ENTITY_TO_CLASS(ammo_handgrenade_box, CGrenadeBox);
