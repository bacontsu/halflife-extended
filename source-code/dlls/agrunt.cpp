...
void CAGrunt :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	...
	case AGRUNT_AE_HORNET1:
		...
		/*if (pev->weapons == AGRUNT_TOXICGUN) <- never implemented, no model
		{
			FireToxic();
		}*/
...
void CAGrunt::Precache()
{
	...
	PRECACHE_SOUND( "agrunt/inferno_cast.wav" );
...
void CAGrunt::FireHornets()
{
	...
	if (Classify() != CLASS_ALIEN_MILITARY)
	{
...
void CAGrunt::FireFireball()
{
	...
	CBaseEntity* pFireball = CBaseEntity::Create("fireball", vecArmPos, UTIL_VecToAngles(vecDirToEnemy), edict());
	UTIL_MakeVectors(pFireball->pev->angles);
	pFireball->pev->velocity = gpGlobals->v_forward * 300;

	EMIT_SOUND_DYN(ENT(pev), CHAN_WEAPON, "agrunt/inferno_cast.wav", 1.0, ATTN_NORM, 0, 100);
}

void CAGrunt::FirePlasma()
{
	if (m_hEnemy == NULL)
	{
		return;
	...
	Vector vecShootOrigin = GetGunPosition() - Vector(0, 0, 5);
...
void CAGrunt::FireToxic()
{
	...
	if (gpGlobals->time - m_flNextHornetAttackCheck <= 0.11)
	{
		return;
	}
	...
	CSpore* pSpore = CSpore::CreateSpore(
		vecShootOrigin, vecShootDir,
		this,
		CSpore::SporeType::ROCKET, false, false);

	UTIL_MakeVectors(vecShootDir);
...
const char* CDeadAGrunt::m_szPoses[] = { "dead_sitting", "dead_front" };
...
void CDeadAGrunt::Spawn()
{
	...
	if (pev->sequence == -1)
	{
		ALERT(at_console, "Dead AGrunt with bad pose\n");
	}
