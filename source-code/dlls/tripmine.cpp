...
void CTripmineGrenade :: PowerupThink()
{
	...
	if (nextWorldAnimUpdate < gpGlobals->time)
	{

		worldSkin++;
		if (worldSkin > 7) { worldSkin = 0; }
		pev->skin = worldSkin;
		//ALERT(at_console, "skin %i", worldSkin);
		nextWorldAnimUpdate = gpGlobals->time + 0.1f;
	}
...
void CTripmineGrenade :: MakeBeam()
{
	...
	m_pBeam->SetColor( 81, 150, 255 );
	m_pBeam->SetScrollRate( 255 );
	m_pBeam->SetBrightness( 80 );
}
...
{
	/*edict_t* pPlayer = pCaller->edict();

	if (m_pRealOwner == pPlayer)
	{
		KillBeam();
		...
		CBaseEntity* pMine;
		pMine = CTripmineGrenade::DropItem("weapon_tripmine", vecMineOrigin, vecMineAngles);
...
{
	// scroll between skins
	if (nextAnimUpdate < gpGlobals->time)
	{
