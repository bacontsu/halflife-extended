...
TYPEDESCRIPTION CBaseButton::m_SaveData[] =
{
	...
	//DEFINE_FIELD(CBaseButton, CustLockedSnd, FIELD_CHARACTER),
	//DEFINE_FIELD(CBaseButton, CustUnLockedSnd, FIELD_CHARACTER),
	DEFINE_FIELD(CBaseButton, nextCustomSndTime, FIELD_FLOAT),
...
void CBaseButton::KeyValue( KeyValueData *pkvd )
{
	...
	else if (FStrEq(pkvd->szKeyName, "CustActivateSnd"))
	{
		CustActivateSnd = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else 
...
{ 
	...
	if (!FStringNull(CustActivateSnd))
	{
		PRECACHE_SOUND(STRING(CustActivateSnd));
		pev->noise = ALLOC_STRING(STRING(CustActivateSnd));
	}
	else
	{
	...
	if (!UTIL_IsMasterTriggered(m_sMaster, pOther))
	{
		if (!FStringNull(CustLockedSnd))
		{
			if (nextCustomSndTime <= gpGlobals->time)
			{
				...
				nextCustomSndTime = gpGlobals->time + 1;
			}
		}
		else PlayLockSounds(pev, &m_ls, TRUE, TRUE);
		return;
	}
...
void CBaseButton::ButtonActivate( )
{
	...
	else
	{
		...
		else PlayLockSounds(pev, &m_ls, FALSE, TRUE);
	}
...
{
	if (FStrEq(pkvd->szKeyName, "CustActivateSnd"))
	{
		CustActivateSnd = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
