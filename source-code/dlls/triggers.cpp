...
void CAutoTrigger::Think()
{
	...
	if (m_SpawnChance <= RANDOM_LONG(1, 100) && m_SpawnChance != 100)
		UTIL_Remove(this);
...
public:
	...
	void EXPORT NDSKillTouch(CBaseEntity* pOther);
	void InitTrigger();
...
class CNDSkillTrigger : public CBaseDelay
{
public:
	void KeyValue(KeyValueData* pkvd) override;
	void Spawn() override;
...
LINK_ENTITY_TO_CLASS(trigger_auto_skill, CNDSkillTrigger);

TYPEDESCRIPTION	CNDSkillTrigger::m_SaveData[] =
{
	DEFINE_FIELD(CNDSkillTrigger, m_globalstate, FIELD_STRING),
	DEFINE_FIELD(CNDSkillTrigger, triggerType, FIELD_INTEGER),
};

IMPLEMENT_SAVERESTORE(CNDSkillTrigger, CBaseDelay);

void CNDSkillTrigger::KeyValue(KeyValueData* pkvd)
{
	if (FStrEq(pkvd->szKeyName, "skill"))
	{
		iSkill = ALLOC_STRING(pkvd->szValue);
		pkvd->fHandled = TRUE;
	}
	else
		CBaseDelay::KeyValue(pkvd);
...
void CNDSkillTrigger::Spawn()
{
	Precache();
}


void CNDSkillTrigger::Precache()
{
	pev->nextthink = gpGlobals->time + 0.1;
}


void CNDSkillTrigger::Think()
{
	gSkillData.iSkillLevel = 4;
		
	
		ALERT(at_console, "\ntrigger_auto_skill sets skill to 4\n");
...
class CTriggerChgFaction : public CBaseEntity
{
public:
	void Spawn() override;
	void Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value, CBaseEntity* pTarget);
};

LINK_ENTITY_TO_CLASS(trigger_changefaction, CTriggerChgFaction);


void CTriggerChgFaction::Spawn()
{
	pev->solid = SOLID_NOT;
}

void CTriggerChgFaction::Use(CBaseEntity* pActivator, CBaseEntity* pCaller, USE_TYPE useType, float value, CBaseEntity* pTarget)
{
	if (!FStringNull(pev->target))
	{
		
		pTarget = UTIL_FindEntityByTargetname(pActivator, STRING(pev->target));
			while (pTarget != NULL)
			{
				CBaseMonster* TargetMonster = pTarget->MyMonsterPointer();
				int ClassInt = pev->body;
				TargetMonster->m_iClass = ClassInt;
				if (TargetMonster->m_hEnemy)
				{
					TargetMonster->m_hEnemy = NULL;
					// make 'em stop attacking... might be better to use a different signal?
					TargetMonster->SetConditions(bits_COND_NEW_ENEMY);
				}
				pTarget = UTIL_FindEntityByTargetname(pActivator, STRING(pev->target));
			}
