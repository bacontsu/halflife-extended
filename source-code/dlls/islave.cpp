...
void CISlave :: Spawn()
{
	...
	m_voicePitch		= RANDOM_LONG( 60, 85 );
	if (g_iSkillLevel == SKILL_HARDEST)
		pev->framerate = 1.6;
...
{
	int i;
	PRECACHE_MODEL("models/gibs/vortigaunt_gibs.mdl");
...
// dead enemy
		...
		if (HasConditions(bits_COND_ENEMY_OCCLUDED) && HasConditions(bits_COND_CAN_RANGE_ATTACK2))
		{
...
void CISlave :: WackBeam( int side, CBaseEntity *pEntity )
{
	...
	m_pBeam[m_iBeams]->SetEndAttachment( side < 0 ? 2 : 1 );
	m_pBeam[m_iBeams]->SetColor( 45, 180, 80 );
...
//=========================================================
// ImpulseAttack - green shockwave
//=========================================================
void CISlave::ImpulseAttack()
{
	...
	WRITE_COORD(pev->origin.x);
	WRITE_COORD(pev->origin.y);
	WRITE_COORD(pev->origin.z + 16 + 256 / .2); // reach damage radius over .3 seconds
	...
	WRITE_COORD(pev->origin.x);
	WRITE_COORD(pev->origin.y);
	WRITE_COORD(pev->origin.z + 16 + (256 / 2) / .2); // reach damage radius over .3 seconds
	WRITE_SHORT(m_iSpriteTexture);
	WRITE_BYTE(0); // startframe
	WRITE_BYTE(0); // framerate
	WRITE_BYTE(2); // life
	...
	while ((pEntity = UTIL_FindEntityInSphere(pEntity, pev->origin, 256)) != NULL)
	{
		...
		{
			if (!FClassnameIs(pEntity->pev, "monster_houndeye") && !FClassnameIs(pEntity->pev, "monster_houndeye_baby"))
			{// houndeyes don't hurt other houndeyes with their attack


					// solo
					flAdjustedDamage = gSkillData.houndeyeDmgBlast * 0.75;

				flDist = (pEntity->Center() - pev->origin).Length();

				flAdjustedDamage -= (flDist / 256) * flAdjustedDamage;
